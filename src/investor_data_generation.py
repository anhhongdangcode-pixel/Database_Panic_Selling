...
def generate_dummy_data(n_rows):
    ...
    for _ in range(n_rows):
        ...
        row = {
            ...
            # ĐÃ XÓA Debug_Type TẠI ĐÂY -> CSV sinh ra sẽ sạch sẽ
        }
        data.append(row)
...
def push_to_mysql(df):
    ...
    try:
        with engine.begin() as conn:
            ...
            print("🧹 [DB] Đang dọn dẹp bảng 'investors'...")
            conn.execute(text("DELETE FROM transactions")) 
            conn.execute(text("DELETE FROM portfolio_history"))
            print("🧹 [DB] Đang dọn dẹp bảng 'investors'...")
            conn.execute(text("DELETE FROM investors"))
            df_to_db = df
            # 3. Đổ dữ liệu vào
            print(f"🚀 [DB] Đang đẩy {len(df)} dòng vào bảng 'investors'...")
